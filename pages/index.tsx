  ...
  useEffect(() => {
    const cargarYFiltrarDatos = async () => {
      try {
        ...
        let datosProcesados = response.data.map(moneda => ({
          ...moneda,
          promedio: (moneda.compra + moneda.venta) / 2,
        })).sort((a, b) => {
          let indexA = ordenMonedas.indexOf(a.nombre);
          let indexB = ordenMonedas.indexOf(b.nombre);
          return (indexA === -1 ? ordenMonedas.length : indexA) - (indexB === -1 ? ordenMonedas.length : indexB);
        });
  ...
  const handleFilter = (filtro?: string) => {
    setResultadosFiltrados(
      monedas.filter(moneda =>
        filtro ? moneda.casa.toLowerCase() === filtro.replaceAll(' ', '').toLowerCase() : true
      )
    );
  };
  ...
  return (
    <>
      ...
      <main className={`${styles.main} ${inter.className}`}>
        {resultadosFiltrados
          .filter((moneda) =>
            moneda.casa.toLowerCase().includes(busqueda.replaceAll(' ', '').toLowerCase())
          )
